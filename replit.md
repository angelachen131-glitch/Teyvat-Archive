...
- Character database with detailed builds and team recommendations
- Artifact set information and character matching
- Interactive team builder with elemental reaction analysis
- Beginner's guide covering progression, resource management, and game mechanics
- Advanced filtering and search capabilities across all content
...
- `/characters/:id` - Detailed character pages
- `/artifacts` - Artifact sets database
- `/team-builder` - Interactive team composition tool
...
- `GET /api/artifacts` - Fetch all artifact sets
- `GET /api/teams` - List saved team compositions
- `POST /api/teams` - Create new team composition
- `DELETE /api/teams/:id` - Remove team composition

**Data Layer**: Currently using in-memory storage (`MemStorage` class) with interfaces designed for future database migration
- Character data includes: element, weapon, rarity, region, role, lore, talents, recommended artifacts/weapons, build priorities, constellations
- Artifact data includes: set name, bonuses (2-piece/4-piece), domain location, best characters
- Team data includes: name, description, character list with roles
...
- Pre-populated with character and artifact data on initialization
...
- Environment variable expected: `DATABASE_URL`
